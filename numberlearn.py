...
# 前向传播
def forward_propagation(X, parameters):  #X:(样本数量，特征数量)的二维数组，parameters时神经网络的参数：权重矩阵和偏置向量（线性参数）
    # 获取参数
    ...
    return Z1, A1, Z2, A2, Z3, A3  # 输出每一层加权输入和激活值，后续继续使用
...
# 反向传播
def backward_propagation(X, Y, parameters, caches):  # cashes 前向传播中计算的中间值，用于反向传播计算梯度。
    m = X.shape[0]  # 先获取样本数量
    W1, b1, W2, b2, W3, b3 = parameters  # 解包权重矩阵和偏执向量
    Z1, A1, Z2, A2, Z3, A3 = caches  # 解包前向传播中的计算中间值
...
def train_model(x_train, y_train, learning_rate=0.001, num_epochs=10, batch_size=128, hidden_layer_size=512):
    ...
    # 初始化参数 没啥好说的，就是根据m.n.num_classes来初始化参数
    np.random.seed(1)
