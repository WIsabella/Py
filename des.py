# 初始置换表IP
IP = [
    58, 50, 42, 34, 26, 18, 10, 2,
    60, 52, 44, 36, 28, 20, 12, 4,
    62, 54, 46, 38, 30, 22, 14, 6,
    64, 56, 48, 40, 32, 24, 16, 8,
    57, 49, 41, 33, 25, 17, 9, 1,
    59, 51, 43, 35, 27, 19, 11, 3,
    61, 53, 45, 37, 29, 21, 13, 5,
    63, 55, 47, 39, 31, 23, 15, 7
]

# 逆初始置换表IP^-1
IP_INV = [
    40, 8, 48, 16, 56, 24, 64, 32,
    39, 7, 47, 15, 55, 23, 63, 31,
    38, 6, 46, 14, 54, 22, 62, 30,
    37, 5, 45, 13, 53, 21, 61, 29,
    36, 4, 44, 12, 52, 20, 60, 28,
    35, 3, 43, 11, 51, 19, 59, 27,
    34, 2, 42, 10, 50, 18, 58, 26,
    33, 1, 41, 9, 49, 17, 57, 25
]

# 密钥初始置换表PC-1
PC_1 = [
    57, 49, 41, 33, 25, 17, 9,
    1, 58, 50, 42, 34, 26, 18,
    10, 2, 59, 51, 43, 35, 27,
    19, 11, 3, 60, 52, 44, 36,
    63, 55, 47, 39, 31, 23, 15,
    7, 62, 54, 46, 38, 30, 22,
    14, 6, 61, 53, 45, 37, 29,
    21, 13, 5, 28, 20, 12, 4
]

# 密钥压缩置换表PC-2
PC_2 = [
    14, 17, 11, 24, 1, 5,
    3, 28, 15, 6, 21, 10,
    23, 19, 12, 4, 26, 8,
    16, 7, 27, 20, 13, 2,
    41, 52, 31, 37, 47, 55,
    30, 40, 51, 45, 33, 48,
    44, 49, 39, 56, 34, 53,
    46, 42, 50, 36, 29, 32
]

# 扩展置换表E
E = [
    32, 1, 2, 3, 4, 5,
    4, 5, 6, 7, 8, 9,
    8, 9, 10, 11, 12, 13,
    12, 13, 14, 15, 16, 17,
    16, 17, 18, 19, 20, 21,
    20, 21, 22, 23, 24, 25,
    24, 25, 26, 27, 28, 29,
    28, 29, 30, 31, 32, 1
]

# S盒
S_BOX = [
    [
        [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
        [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
        [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
        [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]
    ],
    [
        [15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
        [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
        [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
        [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]
    ],
    [
        [10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
        [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
        [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
        [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]
    ],
    [
        [7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
        [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
        [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
        [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]
    ],
    [
        [2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
        [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
        [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
        [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]
    ],
    [
        [12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
        [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
        [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
        [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]
    ],
    [
        [4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
        [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
        [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
        [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]
    ],
    [
        [13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
        [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
        [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
        [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]
    ]
]

# P盒置换表
P = [
    16, 7, 20, 21,
    29, 12, 28, 17,
    1, 15, 23, 26,
    5, 18, 31, 10,
    2, 8, 24, 14,
    32, 27, 3, 9,
    19, 13, 30, 6,
    22, 11, 4, 25
]

# 密钥左移位数表
SHIFT = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]

def string_to_bits(s):
    """将字符串转换为二进制位列表"""
    bits = []
    for char in s:
        # 将字符转换为8位二进制表示
        bin_char = bin(ord(char))[2:].zfill(8)
        bits.extend([int(bit) for bit in bin_char])
    return bits
def hex_to_bits(hex_str):
    """将十六进制字符串转换为二进制位列表"""
    bits = []
    for i in range(0, len(hex_str), 2):
        byte = int(hex_str[i:i+2], 16)
        bits.extend([int(bit) for bit in bin(byte)[2:].zfill(8)])
    return bits
def bits_to_string(bits):
    """将二进制位列表转换为字符串"""
    chars = []
    for i in range(0, len(bits), 8):
        byte = bits[i:i+8]
        char_code = int(''.join(str(bit) for bit in byte), 2)
        chars.append(chr(char_code))
    return ''.join(chars)
def des_decrypt_block(block, subkeys):
    """解密一个64位数据块"""
    block = permute(block, IP)
    left = block[:32]
    right = block[32:]
    
    # 解密时使用子密钥的逆序
    for i in range(15, -1, -1):
        temp_right = right[:]
        f_result = f_function(right, subkeys[i])
        right = [left[j] ^ f_result[j] for j in range(32)]
        left = temp_right
    
    combined = right + left
    plain_block = permute(combined, IP_INV)
    return plain_block
def permute(bits, table):
    """根据置换表进行置换"""
    return [bits[i-1] for i in table]

def left_shift(bits, n):
    """循环左移n位"""
    return bits[n:] + bits[:n]

def generate_subkeys(key_bits):
    """生成16轮子密钥"""
    # 初始密钥置换PC-1
    key_pc1 = permute(key_bits, PC_1)
    
    # 分成左右两部分
    left = key_pc1[:28]
    right = key_pc1[28:]
    
    subkeys = []
    
    for i in range(16):
        # 左右两部分分别循环左移
        left = left_shift(left, SHIFT[i])
        right = left_shift(right, SHIFT[i])
        
        # 合并左右两部分
        combined = left + right
        
        # 压缩置换PC-2，生成子密钥
        subkey = permute(combined, PC_2)
        subkeys.append(subkey)
    
    return subkeys

def f_function(right, subkey):
    """F函数"""
    # 扩展置换E
    expanded = permute(right, E)
    
    # 与子密钥异或
    xor_result = [expanded[i] ^ subkey[i] for i in range(48)]
    
    # S盒替换
    sbox_output = []
    for i in range(8):
        # 取出6位
        block = xor_result[i*6:(i+1)*6]
        row = block[0] * 2 + block[5]  # 行号
        col = block[1] * 8 + block[2] * 4 + block[3] * 2 + block[4]  # 列号
        
        # 从S盒中取值
        s_value = S_BOX[i][row][col]
        
        # 转换为4位二进制
        sbox_output.extend([int(bit) for bit in bin(s_value)[2:].zfill(4)])
    
    # P盒置换
    return permute(sbox_output, P)

def des_encrypt_block(block, subkeys):
    """加密一个64位数据块"""
    # 初始置换IP
    block = permute(block, IP)
    
    # 分成左右两部分
    left = block[:32]
    right = block[32:]
    
    # 16轮Feistel结构
    for i in range(16):
        # 保存当前的右半部分
        temp_right = right[:]
        
        # F函数处理右半部分
        f_result = f_function(right, subkeys[i])
        
        # 左半部分与F函数结果异或
        right = [left[j] ^ f_result[j] for j in range(32)]
        
        # 更新左半部分
        left = temp_right
    
    # 最后交换左右两部分
    combined = right + left
    
    # 逆初始置换IP^-1
    cipher_block = permute(combined, IP_INV)
    
    return cipher_block

def des_encrypt(plaintext, key):
    """DES加密主函数"""
    # 将明文和密钥转换为二进制位
    plaintext_bits = string_to_bits(plaintext)
    key_bits = string_to_bits(key)
    
    # 生成16轮子密钥
    subkeys = generate_subkeys(key_bits)
    
    # 填充明文到8字节的倍数
    padding_length = 8 - (len(plaintext) % 8)
    if padding_length != 8:
        plaintext += chr(padding_length) * padding_length
    
    # 分块加密
    ciphertext_bits = []
    for i in range(0, len(plaintext_bits), 64):
        block = plaintext_bits[i:i+64]
        if len(block) < 64:
            # 填充最后一个块
            block += [0] * (64 - len(block))
        
        encrypted_block = des_encrypt_block(block, subkeys)
        ciphertext_bits.extend(encrypted_block)
    
    # 将二进制位转换为十六进制字符串
    ciphertext_hex = ''.join([hex(int(''.join(map(str, ciphertext_bits[i:i+8])), 2))[2:].zfill(2) 
                            for i in range(0, len(ciphertext_bits), 8)])
    
    return ciphertext_hex.upper()
def des_decrypt(ciphertext_hex, key):
    """DES解密主函数"""
    ciphertext_bits = hex_to_bits(ciphertext_hex)
    key_bits = string_to_bits(key)
    subkeys = generate_subkeys(key_bits)
    
    plaintext_bits = []
    for i in range(0, len(ciphertext_bits), 64):
        block = ciphertext_bits[i:i+64]
        decrypted_block = des_decrypt_block(block, subkeys)
        plaintext_bits.extend(decrypted_block)
    
    # 转换为字符串并去除填充
    plaintext = bits_to_string(plaintext_bits)
    
    # 检查并去除PKCS#7填充
    padding_char = plaintext[-1]
    if ord(padding_char) <= 8:  # 有效的填充字符
        # 验证填充是否正确
        padding_length = ord(padding_char)
        if all(ord(c) == padding_length for c in plaintext[-padding_length:]):
            plaintext = plaintext[:-padding_length]
    
    return plaintext
# 测试
if __name__ == "__main__":
    plaintext = "Big Data Security"
    key = "abcdABCD"  # 8字节密钥
    ciphertext = des_encrypt(plaintext, key)
    print(f"加密结果: {ciphertext}")
    decrypted_text = des_decrypt(ciphertext, key)
    print(f"解密结果: {decrypted_text}")